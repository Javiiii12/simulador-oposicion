...
// Remove all CSIF Tema 15 and 16 to avoid duplicates if run multiple times
qs = qs.filter(q => !(q.origen === 'CSIF' && (q.tema.includes('Tema 15:') || q.tema.includes('Tema 16:'))));
    ...
    blocks.forEach((block, blockIndex) => {
        ...
        for (let i = 0; i < chunks.length - 1; i += 2) {
            ...
            if (ops.length >= 4 && qText.length > 5) {
                ...
                lines = lines.filter(l => !l.toLowerCase().includes('test nº') && !l.toLowerCase().includes('fuente:') && !l.toLowerCase().includes('test tema') && !l.toLowerCase().includes('tema 16 cisf'));
...
const theme15Questions = processFile('c:/Users/34678/Desktop/web-test-pinche/scripts/raw_tema15_csif.txt', 'Maquinaria y Utensilios', 15);
const theme16Questions = processFile('c:/Users/34678/Desktop/web-test-pinche/scripts/raw_tema16_csif.txt', 'Gestión de Residuos', 16);

qs = qs.concat(theme15Questions, theme16Questions);
