...
qs = qs.filter(q => !(q.origen === 'Historico' && q.tema === 'Otras Comunidades: OPE SAS (Andalucía) 2018 + SESCAM 2010'));

const rawText = fs.readFileSync('c:/Users/34678/Desktop/web-test-pinche/scripts/raw_sas_2018.txt', 'utf8');
...
for (let i = 0; i < chunks.length - 1; i += 2) {
    ...
    if (ops.length >= 4 && qText.length > 5) {
        questions.push({
            id: `historico_sas_2018_${i / 2}`,
            tema: "Otras Comunidades: OPE SAS (Andalucía) 2018 + SESCAM 2010",
            pregunta: qText,
            opciones: {
                a: ops[0].trim(),
                b: ops[1].trim(),
...
console.log(`Parsed ${questions.length} questions for SAS 2018.`);
